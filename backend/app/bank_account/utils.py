...
# ? 1 digit: Check digit for security
...
def calculate_luhn_check_digit(number: str) -> int:
    ...
    # ? start from the last element and then step backwards by two
    odd_digits = digits[-1::-2]

    # ? start from the second last element and then step backwards by two
    ...
    # ? ensure that if the result is ten, the check digit is going to be zero
    # ? cuz the check digit can only be single digit
