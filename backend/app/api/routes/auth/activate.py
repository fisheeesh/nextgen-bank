...
from ....auth.schema import AccountStatusSchema, EmailRequestSchema
from ....auth.utils import create_activation_token
...
async def activate_user(token: str, session: SessionDep):
    try:
        ...
        logger.error(f"Failed to activate user accounts: {e}")
        raise HTTPException(
    ...
    try:
        ...
        if user.is_active or user.account_status == AccountStatusSchema.ACTIVE:
            raise HTTPException(
                ...
                detail={
                    "status": "error",
                    "message": "User account aleardy activated",
                    "action": "Please login to your account",
                },
            )
        ...
        return {
            "message": "If an account exists with this email, please check your inbox for the actionvation link"
        }
    except HTTPException as http_ex:
        raise http_ex
    except Exception as e:
