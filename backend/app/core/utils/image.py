...
def validate_image(file_data: bytes) -> Tuple[bool, str]:
    try:
        ...
        with Image.open(image_buffer) as img:
            ...
            if width > settings.MAX_DIENSION or height > settings.MAX_DIENSION:
                return (
                    False,
                    f"Image dimensions exceed {settings.MAX_DIENSION}px limit",
                )
        ...
        return False, "Files is not a valid image"
    except Exception as e:
        logger.error(f"Image validation error: {str(e)}")
        return False, f"Invalid iamge file: {str(e)}"
