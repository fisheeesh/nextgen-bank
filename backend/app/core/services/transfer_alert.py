...
from ...bank_account.enums import AccountCurrencyEnum
...
class TransferAlertEmail(EmailTemplate):
    ...
    template_name_plain = "transfer_alert.html"
...
async def send_transfer_alert(
    *,
    ...
    try:
            ...
            sender_context.update(
                {
                    ...
                    "exchange_rate": format_currency(exchange_rate)
                    if exchange_rate
                    else "1.00",
                    "conversion_fee": format_currency(conversion_fee)
                    if conversion_fee
                    else "0.00",
        ...
        logger.info(
            f"Transfer alerts send successfully. Reference {reference}, "
            f"Sender: {sender_email}, Receiver: {receiver_email}"
        )
    except Exception as e:
        logger.error(
            f"Failed to send transer alerts. Reference: {reference}, Error: {str(e)}"
        )
        raise
