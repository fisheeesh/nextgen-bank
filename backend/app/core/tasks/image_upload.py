...
@celery_app.task(
    ...
    # ? allow the taks to access the request obj
    bind=True,
    max_retries=3,
    soft_time_limit=10,
...
def upload_profile_image_task(
    ...
    try:
        ...
        upload_options = {
            "resource_type": "image",
            "folder": f"{settings.CLOUDINARY_CLOUD_NAME}/profile/{user_id}",
            "public_id": f"{image_type}_{uuid.uuid4()}",
            # ? If the files with the same public ID already exits, it's gonna be overwritten
            "overwrite": True,
        ...
        logger.debug(f"Cloudinary uplaod result: {result}")
        ...
        response: UploadResponse = {
            ...
            "thumbnail_url": (
                # ? second element of the first element of eager
        ...
        for key in ["url", "image_type", "public_id"]:
            if not response.get(key):
                raise Exception(f"Required field {key} missing in upload reponse")
