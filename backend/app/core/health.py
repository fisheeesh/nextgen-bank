...
class ServiceStatus(str, Enum):
    HEALTHY = "healthy"
    UNHEALTHY = "unhealthy"
    # ? functioning but reducted capabilities
    DEGRADED = "degraded"
    STARTING = "starting"
    DOWN = "down"


# ? This will manage the health checks for the variouse services that our application is going to depend on
class HealthCheck:
        ...
        if service_name in self._dependencies:
            for dep in self._dependencies[service_name]:
                ...
                if dep_status != ServiceStatus.HEALTHY:
                    logger.error(
                        f"Dependency {dep} not healty for service {service_name}"
                    )
